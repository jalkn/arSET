...
"@
    ...
    # The -Scope AllUsers parameter is recommended for broader access, but may require admin privileges.
    Install-Module -Name AWSPowerShell.NetCore -Force

    # Explicitly import the module to make cmdlets available in the current session
    Write-Host "📥 Importing AWS Tools for PowerShell..." -ForegroundColor $YELLOW
    Import-Module AWSPowerShell.NetCore

    # Deploy to AWS (New Lines Added Below)
    Write-Host "📦 Zipping project for deployment..." -ForegroundColor $YELLOW

    # Create a zip file of the entire project
    Compress-Archive -Path ".\arpa" -DestinationPath "arpa.zip" -Force

    Write-Host "☁️ Deploying to AWS Elastic Beanstalk..." -ForegroundColor $YELLOW

    # Define deployment variables (replace with your desired names)
    $applicationName = "arpa-app"
    $environmentName = "arpa-env"
    $s3BucketName = "arpa-deployment-bucket-12345" # Must be globally unique

    # Check if the S3 bucket exists, and create it if it doesn't
    if (!(Get-S3Bucket -BucketName $s3BucketName -ErrorAction SilentlyContinue)) {
        Write-Host "Creating S3 bucket: $s3BucketName"
        New-S3Bucket -BucketName $s3BucketName -Region us-east-1 # Change region if needed
    }

# Upload the application archive to S3
    Write-Host "Uploading application archive to S3..."
    Write-S3Object -BucketName $s3BucketName -File ".\arpa.zip" -Key "arpa.zip"

    # Check if the Elastic Beanstalk environment already exists
    if (!(Get-EBEnvironment -EnvironmentName $environmentName -ErrorAction SilentlyContinue)) {
        # If the environment does not exist, create it
        Write-Host "Creating new Elastic Beanstalk application and environment..."
        New-EBEnvironment -ApplicationName $applicationName -EnvironmentName $environmentName -SolutionStackName "64bit Amazon Linux 2023 v4.7.1 running Python 3.13" -OptionSetting @{ Namespace='aws:elasticbeanstalk:environment:tier'; OptionName='Type'; Value='WebServer' } -CnamePrefix "arpa-app" -Force
    }
    else {
        # If the environment exists, create a new version and update it
        Write-Host "Creating Elastic Beanstalk application version..."
        $versionLabel = "version-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
        New-EBApplicationVersion -ApplicationName $applicationName -VersionLabel $versionLabel -SourceBundle_S3Bucket $s3BucketName -SourceBundle_S3Key "arpa.zip"

        Write-Host "Updating existing Elastic Beanstalk environment..."
        Update-EBEnvironment -EnvironmentName $environmentName -ApplicationVersion $versionLabel
    }

    Write-Host "✅ Deployment to AWS initiated successfully. Please check the AWS Management Console for the status." -ForegroundColor $GREEN
